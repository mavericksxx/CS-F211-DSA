...
inline STUDENT get(LINKEDLIST linkedlist, int position) {
    return linkedlist.data[position].entry;
}

inline void display(LINKEDLIST linkedlist) {
    STUDENT student;
...
inline int size(LINKEDLIST linkedlist) {
    return linkedlist.pointer;
}

inline int isEmpty(LINKEDLIST linkedlist) {
    return (linkedlist.pointer == 0) ? 1 : 0;
}
